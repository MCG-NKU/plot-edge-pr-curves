...
n=length(names); hs=zeros(1,n); res=zeros(n,8); prs=cell(1,n);
for i=1:n
...
for i=n:-1:1
  hs(i)=plot(prs{i}(:,1),prs{i}(:,2),'-','LineWidth',2,'Color',colors{i},'LineStyle',lines{i});
  fprintf('ODS=%.3f OIS=%.3f AP=%.3f R50=%.3f',res(i,[3 6:8]));
